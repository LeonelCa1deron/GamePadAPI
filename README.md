...
<head>
    ...
    <title>GamePadAPI</title>
    <script src = "GamePad.js" defer></script>
    <link rel = "shortcut icon" type = "image" href="Icon.png"/>
    <style> .selected-button {fill: white;}</style>
</head>
<body background= "Background.jpg" p style = "text-align: center;">
    <h1> <p style = "color: white; font-family: Verdana, Geneva, Tahoma, sans-serif;"> ¡Welcome to GamePadAPI! </p></h1>
    <h2> <p style = "color: white; font-family: Verdana, Geneva, Tahoma, sans-serif;"> Instruction: First connect your PS4 control for use GamePadAPI. After push the button "Connect" and press any button for begin</p>
      <button onclick = "connect()">Connect</button></h2>
    <svg
    version="1.1"
    id="Capa_1"
...
https://github.com/LeonelCa1deron/GamePadAPI.git

---

# Conclusión
Como conclusión de la práctica, para el funcionamiento de la API se requiere la plantilla del mando de la PS4 el cual es descargada de un repositorio de Github, esta se ubicaría en el código del HTML para que pueda visualizarse al momento de correr el programa; por medio del script, conectamos el archivo .js donde se realizaría el código de funcionamiento. Se implementa una función llamada connect(), el cual consiste en que al momento de conectar el mando y posteriormente de pulse un botón en la página y se empiece a realizar movimiento en el control pueda reflejarse en la pantalla, referente al botón se implementa en el HTML donde el mismo manda a llamar la función en el .js para ejecutar las instrucciones. Gracias a los evento de addListenerEvent la acción de conectar el control se verá reflejada en la consola de la pestaña al igual del mando que se está utilizando y el mensaje dado por la consola gracias al .log donde el usuario podrá reconocer el momento en el que el mando está conectado con la API. Por medio de las funciones: handlebuttons, updatestick, handlestick y gameloop se capturan los movimientos que se realicen en el mando por parte del usuario, y estos al hacer capturados son enviados a la función de gameloop donde constantemente manda a llamar los resultados y después son mostrados en la consola del HTML, en el caso de los updatestick el envío de los resultados es aún mayor ya que al momento de rotar los joystick se obtiene la posición actual en milisegundos, y gracias a ello se podrá observar en la plantilla del mando el movimiento de estos muchos más fluidos por la cantidad de respuestas; mientras menos resultados se obtienen los joystick se verán pocos fluidos en cambio mientras sea mayor serán fluidos. Por último, otras API pueden hacer uso de las funciones fetch() y map() siempre y cuando exista una API Key que es proporcionada por la propia página de la API al llevar a cabo un registro de usuario. Para la función fetch() consiste en hacer un llamado al servidor de la API que se utilizará por medio de la URL, y en la función de map() permite que todos los datos recibidos por parte del servidor llamado por el fetch los guarde en un arreglo.